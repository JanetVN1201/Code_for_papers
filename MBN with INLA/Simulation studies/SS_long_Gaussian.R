...
#=== parameter learning function INLA (default wishart prior) ===#
...
#=== parameter learning function INLA (user defind wishart prior) ===#
...
for(k in 1:nsims) {
  ...
  CPTY6 = list(
    coef = matrix(c(b0, b1, b2, b3), nrow = 4, ncol = nindividual, byrow = TRUE,
                  dimnames = list(c("(Intercept)", "t", "Y1","Y5"), as.character(seq(0, nindividual-1)))),
  ...
  CPTY7 = list(
    coef = matrix(c(b0, b1, b2, b3), nrow = 4, ncol = nindividual, byrow = TRUE,
                  dimnames = list(c("(Intercept)", "t", "Y5","Y6"), as.character(seq(0, nindividual-1)))),
    sd = sde
  )
  data <- data[, !(names(data) %in% c("u0","u1", "e", "b0","b1","b0i","b1i", "b2","b3"))]
  ...
  CPTY9 = list(
    coef = matrix(c(b0, b1, b2, b3, b4), nrow = 5, ncol = nindividual, byrow = TRUE,
                  dimnames = list(c("(Intercept)", "t", "Y1","Y4", "Y8"), as.character(seq(0, nindividual-1)))),
    sd = sde
  )
  ...
  CPTY10 = list(
    coef = matrix(c(b0, b1, b2, b3), nrow = 4, ncol = nindividual, byrow = TRUE,
                  dimnames = list(c("(Intercept)", "t", "Y4","Y6"), as.character(seq(0, nindividual-1)))),
    sd = sde
  )
  data <- data[, !(names(data) %in% c("u0","u1", "e", "b0","b1","b0i","b1i", "b2","b3"))]
  ...
  for (node in nodes(dag.inla3)) {
    if (node == "ID") {
      ...
      local.distributions[[node]] = ldistME_inla3(
        node = node, 
